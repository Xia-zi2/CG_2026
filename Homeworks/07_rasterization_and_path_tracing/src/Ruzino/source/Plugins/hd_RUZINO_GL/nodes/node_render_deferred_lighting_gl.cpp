...
NODE_DECLARATION_FUNCTION(deferred_lighting)
{
    ...
    b.add_input<GLTextureHandle>("Ambient Occlusion");
...
NODE_EXECUTION_FUNCTION(deferred_lighting)
{
    ...
    auto ambient_occlusion =
        params.get_input<GLTextureHandle>("Ambient Occlusion");
    ...
    shader->shader.setInt("ambientOcclusionSampler", 5);
    glActiveTexture(GL_TEXTURE5);
    glBindTexture(GL_TEXTURE_2D, ambient_occlusion->texture_id);
    ...
    for (int i = 0; i < lights.size(); ++i) {
        ...
        frustum.SetPerspective(120.f, 1.0, 1.f, 25.f);
