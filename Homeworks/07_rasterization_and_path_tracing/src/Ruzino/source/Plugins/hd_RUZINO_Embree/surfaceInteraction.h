...
{
    auto wiLocal = WorldToTangent(wi);
    auto woLocal = WorldToTangent(this->wo);
    return material->Eval(wiLocal, woLocal, texcoord);
}

inline float SurfaceInteraction::Pdf(GfVec3f wi, GfVec3f wo) const
{
    auto wiLocal = WorldToTangent(wi);
    auto woLocal = WorldToTangent(wo);
    return material->Pdf(wiLocal, woLocal, texcoord);
}
