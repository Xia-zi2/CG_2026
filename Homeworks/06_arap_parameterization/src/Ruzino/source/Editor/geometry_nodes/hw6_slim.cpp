...
constexpr double kSigmaMin = 1e-6;
constexpr double kMuSafety = 1.001;
constexpr int kLineSearchMax = 20;
constexpr double kEnergyTol = 1e-10;
...
inline void add_face_stiffness_weighted(
    const FaceData& fd,
    double weight,
    ...
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) {
            trips.emplace_back(
                fd.vids[i],
                fd.vids[j],
                fd.area * weight * fd.grad[i].dot(fd.grad[j]));
        }
    }
}

inline void add_face_rhs_weighted(
    const FaceData& fd,
...
{
    for (int i = 0; i < 3; ++i) {
        Eigen::Vector2d rhs_i = fd.area * weight * (L * fd.grad[i]);
...
inline double phi_sd(double sigma)
{
    return sigma * sigma + 1.0 / (sigma * sigma);
}

inline double phi_sd_prime(double sigma)
{
    return 2.0 * (sigma - 1.0 / (sigma * sigma * sigma));
}

inline double phi_sd_second(double sigma)
{
    return 2.0 + 6.0 / (sigma * sigma * sigma * sigma);
}

inline bool compute_slim_local_data(
    ...
    SlimLocalData& out)
{
    ...
    sigma[0] = std::max(sigma[0], kSigmaMin);
    sigma[1] = std::max(sigma[1], kSigmaMin);

    double mu = std::max(phi_sd_second(sigma[0]), phi_sd_second(sigma[1]));
    mu = std::max(mu * kMuSafety, 1.0);

    Eigen::Vector2d sigma_hat;
    sigma_hat[0] = sigma[0] - phi_sd_prime(sigma[0]) / mu;
    sigma_hat[1] = sigma[1] - phi_sd_prime(sigma[1]) / mu;
    sigma_hat[0] = std::max(sigma_hat[0], kSigmaMin);
    sigma_hat[1] = std::max(sigma_hat[1], kSigmaMin);

    out.mu = mu;
    out.L = U * sigma_hat.asDiagonal() * V.transpose();
    return true;
}

inline double slim_energy(
    ...
    const std::vector<Eigen::Vector2d>& uv)
{
    ...
    for (const auto& fd : faces) {
        ...
        energy += fd.area * (phi_sd(sigma[0]) + phi_sd(sigma[1]));
    }
    return energy;
}

inline bool line_search_slim(
    const std::vector<FaceData>& faces,
    const std::vector<Eigen::Vector2d>& uv_prev,
    const std::vector<Eigen::Vector2d>& uv_candidate,
    std::vector<Eigen::Vector2d>& uv_next)
{
    const double E0 = slim_energy(faces, uv_prev);
    if (!std::isfinite(E0)) {
        return false;
    }

    std::vector<Eigen::Vector2d> direction(uv_prev.size(), Eigen::Vector2d::Zero());
    for (int i = 0; i < static_cast<int>(uv_prev.size()); ++i) {
        direction[i] = uv_candidate[i] - uv_prev[i];
    }

    double alpha = 1.0;
    for (int ls = 0; ls < kLineSearchMax; ++ls) {
        std::vector<Eigen::Vector2d> trial = uv_prev;
        for (int i = 0; i < static_cast<int>(trial.size()); ++i) {
            trial[i] += alpha * direction[i];
        }

        double Etrial = slim_energy(faces, trial);
        if (std::isfinite(Etrial) && Etrial <= E0 - kEnergyTol) {
            uv_next = std::move(trial);
            return true;
        }
        alpha *= 0.5;
    }
    ...
    std::vector<Eigen::Vector2d>& uv_out)
{
    ...
    if (!std::isfinite(slim_energy(faces, uv))) {
        // SLIM needs an orientation-preserving initialization.
        return false;
    }
    ...
    double prev_energy = slim_energy(faces, uv);

    for (int iter = 0; iter < max_iter; ++iter) {
        std::vector<SlimLocalData> local(faces.size());
        for (int f = 0; f < static_cast<int>(faces.size()); ++f) {
            if (!compute_slim_local_data(face_jacobian(faces[f], uv), local[f])) {
                return false;
            }
        }
        ...
        for (int f = 0; f < static_cast<int>(faces.size()); ++f) {
            add_face_stiffness_weighted(faces[f], local[f].mu, trips);
            add_face_rhs_weighted(faces[f], local[f].L, local[f].mu, bx, by);
        }
        ...
        std::vector<Eigen::Vector2d> uv_next;
        if (!line_search_slim(faces, uv, uv_candidate, uv_next)) {
            break;
        }

        double next_energy = slim_energy(faces, uv_next);
        ...
        const double rel_drop =
            std::abs(prev_energy - next_energy) / std::max(1.0, std::abs(prev_energy));
...
NODE_DECLARATION_FUNCTION(hw6_slim)
{
    b.add_input<Geometry>("Reference");
    b.add_input<Geometry>("Init");
    b.add_input<int>("Iteration").min(0).max(100).default_val(10);
    b.add_output<Geometry>("SLIM");
}

NODE_EXECUTION_FUNCTION(hw6_slim)
{
    ...
    std::vector<Eigen::Vector2d> uv;
    if (!solve_slim(ref_mesh.get(), init_mesh.get(), max_iter, uv)) {
        return false;
    }
    ...
    params.set_output("SLIM", std::move(*geometry));
    return true;
}

NODE_DECLARATION_UI(hw6_slim);
