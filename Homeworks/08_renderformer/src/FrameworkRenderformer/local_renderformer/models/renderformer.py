...
class RenderFormer(nn.Module, PyTorchModelHubMixin):
    ...
    def construct_seq(self, tri_vpos_list, texture_patch_list, valid_mask, vns):
        ...
        num_tri = tri_vpos_list.size(1)

        # Register tokens are global learnable tokens. They are parameters of
        # shape [1, num_register_tokens, latent_dim] and must be broadcast to
        # the current batch size.
        reg_tokens = self.reg_tokens.expand(batch_size, -1, -1)

        # A shared learnable base token is added to every triangle token.
        tri_token = self.tri_token.expand(batch_size, num_tri, -1)

        if self.config.pe_type == 'nerf':
            # tri_vpos_list is [B, N_tri, 9].  NeRFEncoding expands each
            # coordinate with multi-frequency sin/cos features.  The linear
            # projection maps the resulting high-dimensional positional
            # feature back to latent_dim so it can be added to other embeddings.
            tri_pos_emb = self.tri_encoding_norm(
                self.tri_encoding_proj(self.tri_vpos_pe(tri_vpos_list))
            )
            tri_tokens = tri_token + tri_pos_emb + tri_tex_emb + vn_emb
        elif self.config.pe_type == 'rope':
            # For RoPE, the vertex positions are not added to the token here.
            # They are passed onward as tri_vpos_list and used later to rotate
            # q/k inside attention.  The triangle token is therefore formed
            # from learned base + material/texture + normal information.
            tri_tokens = tri_token + tri_tex_emb + vn_emb
        else:
            raise ValueError(f"Invalid positional encoding type: {self.config.pe_type}")
