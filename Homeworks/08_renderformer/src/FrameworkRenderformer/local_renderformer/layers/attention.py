...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, src_key_padding_mask=None, rope_cos=None, rope_sin=None, rope_ctx_cos=None, rope_ctx_sin=None, force_sdpa=False):
        ...
        if ATTN == 'sdpa' or force_sdpa:
            ...
            # q, k, v have shape [B, H, N, D_head] after projection,
            # multi-head reshaping, and optional RoPE.  The key padding mask
            # uses the local convention True = valid/keep, so invalid padded
            # key positions are masked with -inf before softmax.
            d_k = q.size(-1)
            scores = torch.matmul(q.to(torch.float32), k.to(torch.float32).transpose(-2, -1)) / (d_k ** 0.5)

            if attn_mask is not None:
                scores = scores.masked_fill(~attn_mask, -1e6)

            attn_weights = F.softmax(scores.to(torch.float32), dim=-1).to(scores.dtype)
            ...
            attn_output = (
                ...
                .transpose(1, 2)
                .contiguous()
                .view(bs, src_len, self.num_heads * d_k)
            )
            # ==============================================================
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x, src_key_padding_mask=None, triangle_pos=None):
        ...
        if self.rope_dim is not None:
            ...
            rope_cos, rope_sin = freqs_to_cos_sin(
                rope_freqs,
        ...
        for layer_idx, layer in enumerate(self.layers, start=1):
            if (
                self.encoder_skip_from_layer is not None
                and layer_idx == self.encoder_skip_from_layer
            ):
                skip_x = x.clone()
            ...
            if (
                self.encoder_skip_to_layer is not None
                and layer_idx == self.encoder_skip_to_layer
            ):
                if skip_x is None:
                    raise RuntimeError("encoder skip target reached before source was saved")
                x = x + skip_x
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, x, ctx, src_key_padding_mask=None, triangle_pos=None, ray_pos=None, out_layers=[], tf32_mode=False, patch_h=None, patch_w=None):
        if self.rope_dim is not None:
            ...
            rope_ctx_cos, rope_ctx_sin = freqs_to_cos_sin(
                tri_rope_freqs,
                head_dim=self.head_dim
            )
        else:
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            if layer_idx in out_layers:
                # DPTHead expects each collected feature to be wrapped in a
                # list, matching the structure used by the original code path.
                out_list.append([x])
