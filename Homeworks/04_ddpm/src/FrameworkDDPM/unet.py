...
class SimpleUnet(nn.Module):
    def __init__(self,vocab_size=None):
        ...
        if vocab_size is not None:
            self.tag_embedding = nn.Embedding(vocab_size, time_emb_dim)
        else:
    ...
    def forward(self, x, timestep,cond=None):
        ...
        if cond is not None:
            if self.tag_embedding is None:
                raise ValueError("cond was provided, but tag_embedding is not initialized.")

            cond_emb = self.tag_embedding(cond)   # [B, L, D]
            cond_emb = cond_emb.mean(dim=1)       # [B, D]
            t = t + cond_emb
