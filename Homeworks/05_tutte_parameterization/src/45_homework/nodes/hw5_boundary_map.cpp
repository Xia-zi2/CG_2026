...
template<typename MeshT>
std::vector<int> extract_outer_boundary_loop(MeshT* mesh)
{
...
NODE_EXECUTION_FUNCTION(hw5_circle_boundary_mapping)
{
    ...
    auto boundary_loop = extract_outer_boundary_loop(halfedge_mesh.get());
    if (boundary_loop.size() < 3) {
        return false;
    }

    flatten_all_vertices_to_xy_plane(halfedge_mesh.get());
    map_boundary_to_circle(halfedge_mesh.get(), boundary_loop);
...
NODE_EXECUTION_FUNCTION(hw5_square_boundary_mapping)
{
    ...
    flatten_all_vertices_to_xy_plane(halfedge_mesh.get());
    map_boundary_to_square(halfedge_mesh.get(), boundary_loop);
